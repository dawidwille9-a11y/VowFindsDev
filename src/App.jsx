          ...
          <div className="vf-vendor-detail-sticky" style={{background:'var(--white)',borderRadius:16,padding:24,boxShadow:'var(--card-shadow)',position:'sticky',top:80}}>
            ...
            {isOnRequest(vendor)?(
              <>
                ...
                {overnight>0&&<div style={{display:'inline-flex',alignItems:'center',gap:5,marginTop:10,background:'rgba(196,130,106,0.08)',color:'var(--rose)',fontSize:'0.74rem',padding:'5px 11px',borderRadius:8,border:'1px solid rgba(196,130,106,0.22)'},display:'flex',alignItems:'center',gap:6}}>{IC.calendar(13,'var(--rose)')}Overnight stay may be required</div>}
              </>
            ):(
              <>
                ...
                {overnight>0&&<div style={{display:'inline-flex',alignItems:'center',gap:5,marginTop:12,background:'rgba(196,130,106,0.08)',color:'var(--rose)',fontSize:'0.74rem',padding:'5px 11px',borderRadius:8,border:'1px solid rgba(196,130,106,0.22)'}},{display:'flex',alignItems:'center',gap:6}}>{IC.calendar(13,'var(--rose)')}Overnight stay required</div>}
              </>
            )}
            {!enquirySent?(
              <>
...
function MobileVenueInput({placeholder,onPinned,initialValue='',style={}}) {
  ...
  return <input ref={ref} defaultValue={initialValue} placeholder={placeholder||'Search venue…'} style={{border:'none',background:'transparent',fontFamily:"'DM Sans',sans-serif",fontSize:'1rem',color:'#2c2c2c',outline:'none',width:'100%',...style}}/>;
}
  ...
  return(
    <div onClick={onOpen} style={{background:'#fff',borderRadius:12,marginBottom:10,overflow:'hidden',boxShadow:'0 1px 6px rgba(0,0,0,0.05)',border:'1px solid #f0ebe4',display:'flex',cursor:'pointer',minHeight:100}}>
      ...
      <div style={{width:110,flexShrink:0,background:img?`url(${img}) center/cover`:`linear-gradient(140deg,${vendor.color||'#c8a87a'}cc,${vendor.color||'#c8a87a'}55)`,position:'relative'}}>
        <div style={{position:'absolute',bottom:6,left:6,background:'rgba(58,74,63,0.85)',color:'#e8d5a3',fontSize:'0.6rem',letterSpacing:'0.08em',textTransform:'uppercase',padding:'3px 8px',borderRadius:999,display:'flex',alignItems:'center',gap:4}}><span style={{display:'flex'}}>{(TYPE_ICON[vendor.type]||IC.camera)(11,'#e8d5a3')}</span>{vendor.type}</div>
      </div>
      {/* Content */}
      <div style={{flex:1,padding:'12px 14px',display:'flex',flexDirection:'column',justifyContent:'space-between',minWidth:0}}>
        <div>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.05rem',fontWeight:600,color:'#3a4a3f',marginBottom:2,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{vendor.name}</div>
          <div style={{fontSize:'0.72rem',color:'#a8a8a8',marginBottom:6,display:'flex',alignItems:'center',gap:4}}><span style={{display:'flex'}}>{IC.pin(12,'#a8a8a8')}</span>{vendor.location}{vendor.distance_km?` · ${vendor.distance_km} km`:''}</div>
          {vendor.description&&<div style={{fontSize:'0.75rem',color:'#6b6b6b',lineHeight:1.5,overflow:'hidden',display:'-webkit-box',WebkitLineClamp:2,WebkitBoxOrient:'vertical'}}>{vendor.description}</div>}
        </div>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:8}}>
          <div style={{fontSize:'0.78rem'}}>
            {onReq
              ?<span style={{color:'#c4826a',fontWeight:600,fontStyle:'italic'}}>On Request</span>
              :<span style={{color:'#3a4a3f',fontWeight:700}}>{fmt(total)}</span>}
          </div>
          <div style={{display:'flex',gap:6}}>
            {customerId&&<div onClick={e=>e.stopPropagation()} style={{background:'#f0e8dc',borderRadius:7,width:30,height:30,display:'flex',alignItems:'center',justifyContent:'center'}}><FavStar vendor={vendor} customerId={customerId} size={16}/></div>}
            <button onClick={e=>{e.stopPropagation();onQuote();}} style={{background:'#c4826a',color:'#fff',border:'none',borderRadius:8,padding:'5px 12px',fontSize:'0.72rem',fontWeight:600,cursor:'pointer'}}>Quote</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return(
    <div style={{flex:1,overflowY:'auto',background:'#fff'}}>
      ...
      <div style={{padding:'28px 20px 10px'}}>
        <div style={{fontSize:'0.64rem',letterSpacing:'0.2em',textTransform:'uppercase',color:'#c4826a',marginBottom:8,fontWeight:500}}>Your wedding, your way</div>
        <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'2rem',fontWeight:300,color:'#3a4a3f',lineHeight:1.15,marginBottom:6}}>Find the <em style={{color:'#c4826a',fontStyle:'italic'}}>perfect</em> vendors</h1>
        <p style={{fontSize:'0.82rem',color:'#a8a8a8',lineHeight:1.6,marginBottom:20}}>Enter your venue and we'll calculate real travel costs for every vendor.</p>
      </div>
      ...
      <div style={{margin:'0 16px',background:'#fff',borderRadius:14,padding:'18px',boxShadow:'0 2px 14px rgba(0,0,0,0.06)',position:'relative',zIndex:2,border:'1px solid #ede8e0'}}>
        {/* Venue */}
        <div style={{marginBottom:14}}>
          <div style={{fontSize:'0.65rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'#6b6b6b',marginBottom:6,fontWeight:500}}>Wedding Venue</div>
          <div style={{display:'flex',alignItems:'center',gap:8,background:'#faf6f1',borderRadius:10,padding:'10px 14px',border:`1.5px solid ${venuePinned?'#3a4a3f': '#f0e8dc'}`}}>
            <span style={{flexShrink:0,color:'#a8a8a8',display:'flex'}}>{IC.pin(18,'#a8a8a8')}</span>
            <MobileVenueInput
              placeholder="e.g. Babylonstoren, Franschhoek"
              initialValue={venue}
              ...
              style={{}}
            />
            {venuePinned&&<span style={{fontSize:'0.7rem',color:'#3a4a3f',fontWeight:600,flexShrink:0}}>✓</span>}
          </div>
          {venuePinned&&<div style={{fontSize:'0.68rem',color:'#3a4a3f',marginTop:4,paddingLeft:4}}>Location pinned — distances will be calculated</div>}
        </div>

        {/* Date */}
        <div style={{marginBottom:18}}>
          <div style={{fontSize:'0.65rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'#6b6b6b',marginBottom:6,fontWeight:500}}>Wedding Date (optional)</div>
          <div style={{display:'flex',alignItems:'center',gap:8,background:'#faf6f1',borderRadius:10,padding:'10px 14px', border:'1.5px solid #f0e8dc'}}>
            <span style={{flexShrink:0,color:'#a8a8a8',display:'flex'}}>{IC.calendar(18,'#a8a8a8')}</span>
            <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={{border:'none',background:'transparent',fontFamily:"'DM Sans',sans-serif",fontSize:'0.9rem',color:'#2c2c2c',outline:'none',flex:1}}/>
          </div>
        </div>

        {error&&<div style={{fontSize:'0.78rem',color:'#c4826a',marginBottom:10,padding:'8px 12px',background:'rgba(196,130,106,0.08)',borderRadius:8}}>{error}</div>}

        <button onClick={doSearch} disabled={loading} style={{width:'100%',background:'#3a4a3f',color:'#e8d5a3',border:'none',borderRadius:12,padding:'14px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.95rem',fontWeight:600,cursor:loading?'wait':'pointer',letterSpacing:'0.04em'}}>
      ...
      <div style={{padding:'20px 16px 8px'}}>
        <div style={{fontSize:'0.72rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'#6b6b6b',marginBottom:12,fontWeight:500}}>Browse by category</div>
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          {ALL_TYPES.map(t=>(
            <button key={t} onClick={async()=>{
              ...
              setLoading(false);
            }} style={{background:'#ffffff', border:'1.5px solid #f0e8dc',borderRadius:999,padding:'6px 14px',fontSize:'0.78rem',cursor:'pointer',color:'#3a4a3f',fontWeight:500}}>
<span style={{display:'flex',alignItems:'center',gap:5}}><span style={{display:'flex'}}>{(TYPE_ICON[t]||IC.camera)(14,'#3a4a3f')}</span>{t}</span>
            </button>
          ))}
        </div>
      </div>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      {venuePrompt&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:500,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'24px 20px 32px',width:'100%',maxWidth:480,boxShadow:'0 -8px 40px rgba(0,0,0,0.15)'}}>
            <div style={{width:36,height:4,background:'#e0dbd4',borderRadius:2,margin:'0 auto 20px'}}/>
            <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:12}}>
              {IC.pin(20,'#c4826a')}
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.2rem',fontWeight:600,color:'#3a4a3f'}}>Venue required for accurate pricing</div>
            </div>
            <p style={{fontSize:'0.84rem',color:'#6b6b6b',lineHeight:1.65,marginBottom:20}}>
              Travel costs to <strong>{pendingVendor?.name}</strong> can't be calculated without a venue location. Go back and enter your venue to see accurate pricing.
            </p>
            <button onClick={()=>{setVenuePrompt(false);onBack();}}
              style={{width:'100%',background:'#3a4a3f',color:'#e8d5a3',border:'none',borderRadius:12,padding:'14px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.92rem',fontWeight:600,cursor:'pointer',marginBottom:10}}>
              Add my venue
            </button>
            <button onClick={()=>{setVenuePrompt(false);if(pendingVendor)onOpen(pendingVendor);}}
              style={{width:'100%',background:'none',border:'1.5px solid #e0dbd4',borderRadius:12,padding:'12px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.88rem',color:'#6b6b6b',cursor:'pointer'}}>
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #f0ebe4',padding:'10px 14px',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
        <button onClick={onBack} style={{background:'#f5f0eb',border:'none',borderRadius:8,padding:'6px 8px',color:'#6b6b6b',cursor:'pointer',flexShrink:0,display:'flex',alignItems:'center',gap:4}}>{IC.back(18,'#6b6b6b')}</button>
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:'flex',alignItems:'center',gap:6,fontSize:'0.82rem',fontWeight:600,color:'#3a4a3f',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}><span style={{display:'flex'}}>{IC.pin(14,'#c4826a')}</span>{venue||'All vendors'}</div>
          <div style={{fontSize:'0.7rem',color:'#a8a8a8'}}>{vendors.length} vendors found{dateFrom?` · ${formatDateDisplay(dateFrom)}`:''}</div>
        </div>
      </div>

      {/* Search bar */}
      <div style={{background:'#fff',padding:'10px 14px', borderBottom:'1px solid #f0e8dc',flexShrink:0}}>
        <div style={{background:'#faf6f1',borderRadius:10,padding:'8px 14px',display:'flex',alignItems:'center',gap:8}}>
          <span style={{display:'flex'}}>{IC.search(16,'#a8a8a8')}</span>
          <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search vendors…" style={{border:'none',background:'transparent',fontFamily:"'DM Sans',sans-serif",fontSize:'0.88rem',color:'#2c2c2c',outline:'none',flex:1}}/>
          {search&&<button onClick={()=>setSearch('')} style={{background:'none',border:'none',color:'#a8a8a8',cursor:'pointer',padding:0,display:'flex'}}>{IC.x(14,'#a8a8a8')}</button>}
        </div>
      </div>

      {/* Type filter pills */}
      <div style={{background:'#fff',padding:'8px 14px 10px',display:'flex',gap:8,overflowX:'auto',scrollbarWidth:'none',flexShrink:0, borderBottom:'1px solid #f0e8dc'}}>
        <button onClick={()=>setActiveType(null)} style={{flexShrink:0,background:!activeType?'#3a4a3f': '#f0e8dc',color:!activeType?'#e8d5a3':'#6b6b6b',border:'none',borderRadius:999,padding:'5px 14px',fontSize:'0.76rem',fontWeight:500,cursor:'pointer'}}>All</button>
        {types.map(t=>(
          <button key={t} onClick={()=>setActiveType(activeType===t?null:t)} style={{flexShrink:0,background:activeType===t?'#3a4a3f': '#f0e8dc',color:activeType===t?'#e8d5a3':'#6b6b6b',border:'none',borderRadius:999,padding:'5px 14px',fontSize:'0.76rem',fontWeight:500,cursor:'pointer'}}>
            {TYPE_EMOJI[t]} {t}
          </button>
        ))}
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'12px 14px',background:'#fff'}}>
        {filtered.length===0
          ...
          :filtered.map(v=>(
            <MobileVendorCard key={v.id} vendor={v} onOpen={()=>handleOpen(v)} onQuote={()=>onQuote(v)} customerId={customerId}/>
          ))
        }
        <div style={{height:20}}/>
      </div>
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{height:240,position:'relative',background:imgs[imgIdx]?.url?`url(${imgs[imgIdx].url}) center/cover`:`linear-gradient(140deg,${vendor.color||'#c8a87a'}cc,${vendor.color||'#c8a87a'}55)`,flexShrink:0}}>
        ...
        <div style={{position:'absolute',top:12,right:12,background:'rgba(58,74,63,0.85)',color:'#e8d5a3',fontSize:'0.65rem',letterSpacing:'0.1em',textTransform:'uppercase',padding:'3px 10px',borderRadius:999}}>{TYPE_EMOJI[vendor.type]} {vendor.type}</div>
      </div>

      {/* Name + price strip */}
      <div style={{background:'#fff',padding:'14px 16px 0',flexShrink:0}}>
        <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10,marginBottom:4}}>
          <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.5rem',fontWeight:600,color:'#3a4a3f',lineHeight:1.2,flex:1}}>{vendor.name}</div>
          <div style={{textAlign:'right',flexShrink:0}}>
            {onReq?<div style={{fontSize:'0.88rem',color:'#c4826a',fontWeight:700,fontStyle:'italic'}}>On Request</div>:<div style={{fontSize:'1.1rem',color:'#c4826a',fontWeight:700}}>{fmt(total)}</div>}
            {venueLabel&&<div style={{fontSize:'0.68rem',color:'#a8a8a8',marginTop:2}}>from {venueLabel}</div>}
          </div>
        </div>
        <div style={{fontSize:'0.76rem',color:'#a8a8a8',marginBottom:12}}>📍 {vendor.location}{vendor.distance_km?` · ${vendor.distance_km} km away`:''}</div>

        {/* Tabs */}
        <div style={{display:'flex', borderBottom:'1px solid #f0e8dc',gap:0}}>
          {['about','pricing','availability'].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={{flex:1,background:'none',border:'none',borderBottom:`2px solid ${tab===t?'#3a4a3f':'transparent'}`,padding:'8px 4px',fontSize:'0.78rem',fontWeight:tab===t?600:400,color:tab===t?'#3a4a3f':'#a8a8a8',cursor:'pointer',textTransform:'capitalize'}}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div style={{flex:1,overflowY:'auto',background:'#fafafa',padding:'14px 16px'}}>
        {tab==='about'&&(
          <div>
            {vendor.description&&<div style={{background:'#fff',borderRadius:12,padding:'14px',marginBottom:12,fontSize:'0.86rem',color:'#2c2c2c',lineHeight:1.7}}>{vendor.description}</div>}
            {vendor.extra_info&&<div style={{background:'#fff',borderRadius:12,padding:'14px',marginBottom:12}}>
              <div style={{fontSize:'0.68rem',textTransform:'uppercase',letterSpacing:'0.1em',color:'#6b6b6b',marginBottom:8,fontWeight:500}}>Additional Info</div>
              <div style={{fontSize:'0.84rem',color:'#6b6b6b',lineHeight:1.7}}>{vendor.extra_info}</div>
            </div>}
            {vendor.instagram&&(
              <a href={`https://instagram.com/${vendor.instagram.replace('@','')}`} target="_blank" rel="noreferrer" style={{display:'flex',alignItems:'center',gap:10,background:'#fff',borderRadius:12,padding:'12px 14px',textDecoration:'none',marginBottom:12}}>
                ...
                <div>
                  <div style={{fontSize:'0.8rem',fontWeight:600,color:'#3a4a3f'}}>@{vendor.instagram.replace('@','')}</div>
                  <div style={{fontSize:'0.7rem',color:'#a8a8a8'}}>View Instagram</div>
                </div>
              </a>
            )}
          </div>
          ...
          <div>
            {onReq?(
              <div style={{background:'#fff',borderRadius:12,padding:'14px',marginBottom:12}}>
                <div style={{fontSize:'0.92rem',fontWeight:600,color:'#3a4a3f',marginBottom:8}}>Pricing on Request</div>
                <div style={{fontSize:'0.84rem',color:'#6b6b6b',lineHeight:1.7,marginBottom:12}}>This vendor's pricing varies based on your specific requirements. Request a quote to get a personalised price.</div>
                {vendor.distance_km>0&&<div style={{display:'flex',justifyContent:'space-between',fontSize:'0.82rem',padding:'8px 0', borderTop:'1px solid #f0e8dc'}}><span style={{color:'#6b6b6b'}}>Travel ({vendor.distance_km} km)</span><span style={{fontWeight:600}}>{fmt(travel)}</span></div>}
                {overnight>0&&<div style={{display:'flex',justifyContent:'space-between',fontSize:'0.82rem',padding:'8px 0', borderTop:'1px solid #f0e8dc'}}><span style={{color:'#6b6b6b'}}>Overnight fee</span><span style={{fontWeight:600}}>{fmt(overnight)}</span></div>}
              </div>
            ):(
              <div style={{background:'#fff',borderRadius:12,padding:'14px',marginBottom:12}}>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:'0.84rem',padding:'8px 0'}}><span style={{color:'#6b6b6b'}}>Base rate</span><span style={{fontWeight:500}}>{fmt(vendor.fixed_rate)}</span></div>
                <div style={{display:'flex',justifyContent:'space-between',fontSize:'0.84rem',padding:'8px 0', borderTop:'1px solid #f0e8dc'}}><span style={{color:'#6b6b6b'}}>Travel ({vendor.distance_km||0} km × R{vendor.per_km_rate}/km)</span><span style={{fontWeight:500}}>{fmt(travel)}</span></div>
                {overnight>0&&<div style={{display:'flex',justifyContent:'space-between',fontSize:'0.84rem',padding:'8px 0', borderTop:'1px solid #f0e8dc'}}><span style={{color:'#6b6b6b'}}>Overnight fee</span><span style={{fontWeight:500}}>{fmt(overnight)}</span></div>}
                <div style={{display:'flex',justifyContent:'space-between',fontSize:'0.92rem',padding:'12px 0 4px',borderTop:'2px solid #f0e8dc',marginTop:4}}><span style={{color:'#3a4a3f',fontWeight:700}}>Estimated total</span><span style={{color:'#c4826a',fontWeight:700}}>{fmt(total)}</span></div>
              </div>
            )}
          </div>
        )}
        ...
        {tab==='availability'&&(
          <div style={{background:'#fff',borderRadius:12,padding:'14px'}}>
            <div style={{fontSize:'0.84rem',color:'#6b6b6b',marginBottom:12}}>Check available dates for this vendor below.</div>
      ...
      {/* Sticky CTA */}
      <div style={{background:'#fff',padding:'12px 16px', borderTop:'1px solid #f0e8dc',flexShrink:0}}>
        <button onClick={onQuote} style={{width:'100%',background:'#c4826a',color:'#fff',border:'none',borderRadius:14,padding:'15px',fontFamily:"'DM Sans',sans-serif",fontSize:'1rem',fontWeight:600,cursor:'pointer',letterSpacing:'0.04em'}}>
          💌 Request a Quote
        </button>
      </div>
    </div>
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #f0ebe4',padding:'14px 16px 12px',flexShrink:0}}>
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
          <div>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.4rem',color:'#e8d5a3',fontWeight:600}}>My Quotes</div>
            <div style={{fontSize:'0.72rem',color:'rgba(255,255,255,0.5)',marginTop:2}}>Hi, {user.name}</div>
          </div>
          {unread>0&&<div style={{background:'#c4826a',color:'#fff',borderRadius:999,fontSize:'0.72rem',fontWeight:700,padding:'3px 10px'}}>{unread} new</div>}
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:'auto',background:'#fff',padding:'12px 14px'}}>
        {loading?(
          ...
          <div style={{textAlign:'center',padding:'60px 20px'}}>
            <div style={{fontSize:'3rem',marginBottom:12}}>💌</div>
            <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.3rem',color:'#3a4a3f',marginBottom:8}}>No quotes yet</div>
            <p style={{fontSize:'0.84rem',color:'#6b6b6b',marginBottom:20,lineHeight:1.6}}>Browse vendors and tap "Request a Quote" to start a conversation.</p>
            <button onClick={onBrowse} style={{background:'#3a4a3f',color:'#e8d5a3',border:'none',borderRadius:10,padding:'12px 24px',fontSize:'0.88rem',fontWeight:600,cursor:'pointer'}}>Browse Vendors</button>
          </div>
        ):(
          orderedTypes.map(type=>(
            <div key={type} style={{marginBottom:22}}>
              <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                <span style={{fontSize:'1.1rem'}}>{TYPE_EMOJI[type]||'💼'}</span>
                <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.1rem',fontWeight:600,color:'#3a4a3f'}}>{type}</span>
                <span style={{fontSize:'0.68rem',color:'#a8a8a8',background:'#f0e8dc',padding:'2px 8px',borderRadius:999}}>{grouped[type].length}</span>
              </div>
              {grouped[type].map(lead=>{
                ...
                return(
                  <div key={lead.id} onClick={()=>setActiveLead(lead)}
                    style={{background:'#fff',borderRadius:12,padding:'12px 14px',marginBottom:8,cursor:'pointer',display:'flex',alignItems:'center',gap:12,border:`1px solid ${hasNew?'rgba(196,130,106,0.3)':'#f0ebe4'}`,borderLeft:`3px solid ${hasNew?'#c4826a':'#f0ebe4'}`}}>
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      <div style={{display:'flex',alignItems:'center',gap:6,marginBottom:1}}>
                        <span style={{fontWeight:600,fontSize:'0.88rem',color:'#3a4a3f',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{lead.vendor?.name}</span>
                        {hasNew&&<span style={{width:7,height:7,borderRadius:'50%',background:'#c4826a',flexShrink:0,display:'inline-block'}}/>}
                      </div>
                      <div style={{fontSize:'0.72rem',color:'#6b6b6b',marginBottom:hasNew?3:0}}>{lead.title}</div>
                      {lead.last_message&&<div style={{fontSize:'0.71rem',color:'#a8a8a8',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{lead.last_message.sender_role==='vendor'?'Vendor: ':''}{lead.last_message.message_text||'Attachment'}</div>}
                    </div>
                    <div style={{textAlign:'right',flexShrink:0}}>
                      ...
                      <span style={{fontSize:'0.65rem',color:'#a8a8a8'}}>{lead.created_at?new Date(lead.created_at).toLocaleDateString('en-ZA',{day:'numeric',month:'short'}):''}</span>
                    </div>
                  </div>
                );
              })}
