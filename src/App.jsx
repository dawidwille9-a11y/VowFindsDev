...
function MobileSearchScreen({onVendorsLoaded,initialVenue='',initialLL=null}) {
  ...
  return(
    <div style={{flex:1,overflowY:'auto',background:'#fff'}}>
      {/* Hero */}
      <div style={{background:'linear-gradient(160deg,#2c3d32 0%,#1e2820 100%)',padding:'36px 20px 28px',position:'relative',overflow:'hidden'}}>
      ...
      {/* Search card */}
      <div style={{margin:'0 16px',marginTop:-16,background:'#fff',borderRadius:16,padding:'18px',boxShadow:'0 4px 20px rgba(0,0,0,0.08)',position:'relative',zIndex:2,border:'1px solid #f0ebe4'}}>
        {/* Venue */}
        <div style={{marginBottom:14}}>
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #f0ebe4',padding:'10px 14px',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
        <button onClick={onBack} style={{background:'#f5f0eb',border:'none',borderRadius:8,padding:'6px 10px',color:'var(--mid)',cursor:'pointer',fontSize:'0.8rem',flexShrink:0}}>‹ Back</button>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:'0.82rem',fontWeight:600,color:'var(--forest)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>📍 {venue||'All vendors'}</div>
...
function MobileApp({user,onLogin,onLogout,requestQuote,quoteVendor,setQuoteVendor,newLeadAfterQuote,setNewLeadAfterQuote}) {
  ...
  const MENU_ITEMS=[
    ...
    {id:'browse',emoji:'🔍', label:'Browse Vendors'},
    {id:'quotes',emoji:'💬', label:'My Quotes'},
    {id:'plan',  emoji:'💍', label:'Wedding Plan'},
  ];
  ...
  return(
    <div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',background:'#fff',fontFamily:"'DM Sans',sans-serif"}}>
      <GlobalStyles/>
            ...
            <div style={{flex:1,padding:'8px 0'}}>
              {MENU_ITEMS.map(item=>(
                <button key={item.id} onClick={()=>goScreen(item.id)}
                  style={{width:'100%',textAlign:'left',background:screen===item.id?'rgba(58,74,63,0.06)':'none',border:'none',borderLeft:`3px solid ${screen===item.id?'var(--forest)':'transparent'}`,padding:'14px 20px',fontSize:'0.9rem',color:screen===item.id?'var(--forest)':'var(--charcoal)',fontWeight:screen===item.id?600:400,cursor:'pointer',display:'flex',alignItems:'center',gap:12,fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <div style={{background:'#fff',borderBottom:'1px solid #f0ebe4',display:'flex',alignItems:'center',justifyContent:'space-between',padding:'0 16px',height:52,flexShrink:0,paddingTop:'env(safe-area-inset-top)',boxShadow:'0 1px 6px rgba(0,0,0,0.04)'}}>
        {/* Burger */}
        <button onClick={()=>setMenuOpen(o=>!o)} style={{background:'none',border:'none',cursor:'pointer',padding:'6px',display:'flex',flexDirection:'column',gap:4.5,borderRadius:8}}>
          <div style={{width:20,height:1.5,background:'var(--charcoal)',borderRadius:2}}/>
          <div style={{width:14,height:1.5,background:'var(--charcoal)',borderRadius:2}}/>
          <div style={{width:20,height:1.5,background:'var(--charcoal)',borderRadius:2}}/>
        </button>
        {/* Brand */}
      ...
      <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
        ...
        {screen==='browse'&&(
          vendors.length>0
            ?<MobileResultsScreen vendors={vendors} venue={venue} venueLL={venueLL} dateFrom={dateFrom}
                onBack={()=>{setScreen('home');setShowSearch(true);}} onOpen={setActiveVendor} onQuote={handleRequestQuote}
                customerId={user?.customerId} activeType={activeType} setActiveType={setActiveType}/>
            :<MobileSearchScreen onVendorsLoaded={handleVendorsLoaded}/>
        )}
        {screen==='quotes'&&(
          ...
          <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
            <WeddingPlan vendors={vendors} onClose={()=>setScreen('home')}
              onSearchVendors={({venueName,venueLL:ll,type})=>{
                setVenue(venueName||venue);if(ll)setVenueLL(ll);setActiveType(type);setScreen('browse');
                supaFetch('vendors?select=*,images:vendor_images(*),unavail_dates:vendor_unavailable_dates(date)&order=type,name')
                  .then(async data=>{if(ll){const kms=await getBatchDistancesKm(ll,data);setVendors(data.map((v,i)=>({...v,distance_km:kms[i]||0})));}else setVendors(data);}).catch(()=>{});
              }}/>
          </div>
        )}
      </div>
