...
import { supaFetch, supabase } from '../../api.js';
...
function ChatThread({lead,currentRole,currentName,onBack}) {
  ...
  useEffect(()=>{
    // Initial load
    loadMessages();
    // When vendor opens a new lead, mark it as responded so the badge clears
    if(currentRole==='vendor'&&lead.status==='new'){
      supaFetch(`leads?id=eq.${lead.id}`,{method:'PATCH',body:JSON.stringify({status:'responded'}),prefer:'return=minimal'}).catch(()=>{});
      setLeadStatus('responded');
    }

    // Realtime subscription — fires instantly when a new message is inserted
    const channelName=`messages:${lead.id}:${currentRole}`;
    channelRef.current=supabase
      ...
      .on('postgres_changes',{
        ...
        // New message arrived — add it to state if not already there
        setMessages(prev=>{
          if(prev.some(m=>m.id===payload.new.id))return prev;
          // Remove matching optimistic message if present
          const filtered=prev.filter(m=>!m._sending||m.sender_role!==payload.new.sender_role);
          return [...filtered, payload.new];
        });
        // Also refresh typing/read state
        loadTypingAndRead();
      })
      .subscribe();
  ...
  async function loadTypingAndRead(){
    try{
  ...
  async function sendMessage(e){
    ...
    try{
      ...
      // Auto-update status: vendor reply → 'responded', customer reply → keep or set 'new'
      if(currentRole==='vendor'&&leadStatus==='new'){
        await supaFetch(`leads?id=eq.${lead.id}`,{method:'PATCH',body:JSON.stringify({status:'responded'}),prefer:'return=minimal'});
        setLeadStatus('responded');
      } else if(currentRole==='customer'&&leadStatus==='closed'){
        await supaFetch(`leads?id=eq.${lead.id}`,{method:'PATCH',body:JSON.stringify({status:'new'}),prefer:'return=minimal'});
        setLeadStatus('new');
      }
      loadMessages();
    }catch(err){
  ...
  const STATUS_META={
    new:{label:'New',bg:'#ffffff',color:'#c9600a',dot:'#e8852a'},
    responded:{label:'Responded',bg:'#ffffff',color:'#1a7a4a',dot:'#2aaa6a'},
    closed:{label:'Closed',bg:'rgba(255,255,255,0.25)',color:'#ffffff',dot:'rgba(255,255,255,0.7)'},
  };
        ...
        <div style={{display:'flex',alignItems:'center',gap:6,background:sm.bg,borderRadius:999,padding:'4px 10px',flexShrink:0}}>
          ...
          {currentRole==='vendor'?(
            <select value={leadStatus} onChange={e=>updateStatus(e.target.value)}
              style={{border:'none',background:'transparent',fontSize:'0.72rem',fontWeight:700,color:sm.color,cursor:'pointer',outline:'none',padding:0,minWidth:80}}>
