...
import { ON_REQUEST_TYPES, ON_REQUEST_QUESTIONS } from '../../constants.js';
import { isOnRequest, calcPackageTotal, fmt } from '../../utils.js';
  ...
  async function submit(e){
    ...
    try{
      ...
      const msgLines = onReq ? [
        ...
        '',
        ...(selectedPkg?[`Interested in package: ${selectedPkg.name} (${fmt(selectedPkg.fixed_rate)} base)`,'']:[]),
        'Project: ' + form.title,
        '',
        form.description,
  ...
  return(
    <div ref={backdropRef} onMouseDown={handleBackdropMouseDown} onMouseUp={handleBackdropMouseUp} style={{position:'fixed',inset:0,background:'rgba(22,32,24,0.6)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',backdropFilter:'blur(4px)',padding:'20px'}}>
      <div style={{background:'var(--white)',borderRadius:20,padding:36,width:520,maxWidth:'100%',maxHeight:'90vh',overflowY:'auto',boxShadow:'0 24px 80px rgba(0,0,0,0.28)',position:'relative'}}>
        ...
        {/* Package selector — shown when vendor has multiple packages */}
        {!onReq&&vendor.packages&&vendor.packages.length>0&&(
          <div style={{marginBottom:20}}>
            <div style={{fontSize:'0.72rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--mid)',marginBottom:10,fontWeight:500}}>
              {vendor.packages.length>1?'Select a Package':'Package'}
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:8}}>
              {[...vendor.packages]
                ...
                .map(pkg=>{
                  ...
                  return(
                    <div key={pkg.id} onClick={()=>setSelectedPkg(sel?null:pkg)}
                      style={{borderRadius:10,padding:'10px 14px',cursor:'pointer',
                        border:`2px solid ${sel?'var(--forest)':'var(--parchment)'}`,
                        background:sel?'rgba(58,74,63,0.04)':'var(--cream)',
                        transition:'all 0.15s'}}>
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                        <div style={{display:'flex',alignItems:'center',gap:8}}>
                          <div style={{width:16,height:16,borderRadius:'50%',border:`2px solid ${sel?'var(--forest)':'var(--parchment)'}`,background:sel?'var(--forest)':'transparent',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                            {sel&&<div style={{width:6,height:6,borderRadius:'50%',background:'var(--gold-light)'}}/>}
                          </div>
                          <span style={{fontSize:'0.88rem',fontWeight:sel?600:400,color:'var(--forest)'}}>{pkg.name}</span>
                        </div>
                        <span style={{fontSize:'0.88rem',fontWeight:600,color:'var(--rose)'}}>{fmt(pkgT)}</span>
                      </div>
                      {pkg.description&&<p style={{fontSize:'0.75rem',color:'var(--mid)',margin:'4px 0 0 24px',lineHeight:1.5}}>{pkg.description}</p>}
                    </div>
                  );
                })}
            </div>
