...
import { fmt, formatDateDisplay, isOnRequest, isVendorUnavailable, calcPackageTotal, calcFromPrice, cityFromLocation } from '../../utils.js';
import { TYPE_EMOJI } from '../../constants.js';
import FavStar from './FavStar.jsx';
import { TYPE_ICON } from '../../icons.jsx';

const VendorCard=memo(function VendorCard({vendor,unavail,dateFrom,dateTo,onClick,onRequestQuote,customerId=null,onFav=false,onFavRemoved=null}) {
  const travel=(vendor.distance_km||0)*(vendor.per_km_rate||0),overnight=(vendor.distance_km||0)>(vendor.overnight_threshold_km||80)?(vendor.overnight_fee||0):0,total=(vendor.fixed_rate||0)+travel+overnight,primaryImg=vendor.display_picture||vendor.images?.[0]?.url;
  return (
...
<div style={{display:'flex',justifyContent:'center'}}>{IC.calendar(32,'#f0e8dc')}</div>
      ...
      <div className="vf-card-img" style={{height:160,position:'relative',background:primaryImg?`url(${primaryImg}) center/cover`:`linear-gradient(140deg,${vendor.color||'#c8a87a'}ee 0%,${vendor.color||'#c8a87a'}66 100%),linear-gradient(160deg,#ede5db 0%,#d8ccc0 100%)`}}>
        ...
        <div style={{position:'absolute',top:8,right:8,display:'flex',gap:4,alignItems:'center'}}>
          {onFav&&<div onClick={e=>{e.stopPropagation();}} style={{background:'rgba(255,255,255,0.92)',borderRadius:7,width:28,height:28,display:'flex',alignItems:'center',justifyContent:'center'}}><FavStar vendor={vendor} customerId={customerId} onFavRemoved={onFavRemoved}/></div>}
      ...
      <div className="vf-card-body" style={{padding:'16px 18px 18px'}}>
        ...
        <div className="vf-card-location" style={{fontSize:'0.75rem',color:'var(--light)',marginBottom:10,display:'flex',alignItems:'center',gap:4}}><span style={{display:'flex'}}>{IC.pin(13,'var(--light)')}</span>{cityFromLocation(vendor.location)}{vendor.distance_km?` · ${vendor.distance_km} km away`:''}</div>
        ...
        {isOnRequest(vendor)?(
          ...
          const fromPrice=calcFromPrice(vendor);
          if(fromPrice){
            // Has packages — show from price
            return(
              <div className="vf-card-pricing" style={{background:'var(--cream)',borderRadius:9,padding:'10px 12px'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                  <div>
                    <div style={{fontSize:'0.68rem',color:'var(--mid)',textTransform:'uppercase',letterSpacing:'0.07em',marginBottom:2}}>
                      {vendor.packages.length} package{vendor.packages.length!==1?'s':''}
                    </div>
                    <div style={{fontSize:'0.82rem',fontWeight:700,color:'var(--rose)'}}>
                      {fromPrice.isFrom?'From ':''}{fmt(fromPrice.price)}
                    </div>
                  </div>
                  <div style={{fontSize:'0.68rem',color:'var(--light)',textAlign:'right'}}>
                    {vendor.distance_km?`incl. ${vendor.distance_km}km travel`:''}
                  </div>
                </div>
              </div>
            );
