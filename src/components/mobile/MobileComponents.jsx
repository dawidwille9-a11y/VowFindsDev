...
import { fmt, formatDateDisplay, isOnRequest, isVendorUnavailable, calcTotal, calcPackageTotal, calcFromPrice, cityFromLocation } from '../../utils.js';
...
function MobileVenueInput({placeholder, onPinned, onTyped, initialValue='', style={}}) {
  const ref = useRef();
  useEffect(()=>{
      ...
      <div style={{flex:1,padding:'11px 12px',display:'flex',flexDirection:'column',justifyContent:'space-between',minWidth:0}}>
        <div>
          ...
          <div style={{fontSize:'0.71rem',color:M.muted,marginBottom:5,display:'flex',alignItems:'center',gap:4}}>
            {IC.pin(11, M.muted)}{cityFromLocation(vendor.location)}{vendor.distance_km?` · ${vendor.distance_km} km`:''}
          </div>
...
function MobileSearchScreen({onVendorsLoaded}) {
  ...
  async function doSearch() {
    if(!venue.trim()){setError('Please enter your wedding venue to search'); return;}
    ...
    try{
      ...
      if(!data||data.length===0){
        onVendorsLoaded([], venue, venueLL, dateFrom);
        return;
      }
      if(venueLL){
        try{
          ...
          // Maps failed — still show vendors, just without distances
          onVendorsLoaded(data.map(v=>({...v,distance_km:0})), venue, venueLL, dateFrom);
        }
      } else {
        onVendorsLoaded(data, venue, venueLL, dateFrom);
      }
    }catch(e){setError('Could not load vendors: '+e.message);}
    setLoading(false);
  }
  ...
  return(
    <div style={{flex:1,overflowY:'auto',background:M.offWhite}}>
      ...
      <div style={{margin:'12px 16px',background:M.white,borderRadius:14,padding:'18px',
        ...
        <div style={{marginBottom:14}}>
          ...
          <div style={{display:'flex',alignItems:'center',gap:8,background:M.offWhite,borderRadius:10,
            ...
            <MobileVenueInput placeholder="e.g. Babylonstoren, Franschhoek"
              ...
              onTyped={(text)=>{setVenue(text);if(!text)setVenuePinned(false);}}/>
        ...
        {error&&<div style={{fontSize:'0.78rem',color:M.rose,marginBottom:10,padding:'10px 12px',
          background:'rgba(196,130,106,0.06)',borderRadius:8,border:`1px solid rgba(196,130,106,0.2)`}}>
          <div style={{fontWeight:600,marginBottom:2}}>Could not load vendors</div>
          <div style={{fontSize:'0.72rem',opacity:0.85,wordBreak:'break-all'}}>{error.replace('Could not load vendors: ','')}</div>
        </div>}
        ...
        <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
          {ALL_TYPES.map(t=>(
            <button key={t} onClick={async()=>{
              ...
              }catch(e){setError('Could not load vendors: '+e.message||'Could not load.');}
              setLoading(false);
    ...
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:M.offWhite}}>
      ...
      <div style={{background:M.white,padding:'14px 16px 0',flexShrink:0}}>
        ...
        <div style={{fontSize:'0.76rem',color:M.muted,marginBottom:12,display:'flex',alignItems:'center',gap:4}}>
          {IC.pin(13,M.muted)}{cityFromLocation(vendor.location)}{vendor.distance_km?` · ${vendor.distance_km} km away`:''}
        </div>
        {/* Tabs */}
