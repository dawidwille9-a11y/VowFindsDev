...
function isVendorUnavailable(vendor, dateFrom, dateTo) {
...
function CustomerDashboard({user,onLogout,onBrowse,initialLead=null}) {
  ...
  useEffect(()=>{
    loadLeads();
    // Realtime: listen for new messages and lead updates
    channelRef.current=supabase
      .channel(`dashboard:${user.customerId}`)
  ...
  return (
    <div className="vf-lane-wrapper" style={{marginBottom:12,background:'#ffffff'}}>
      ...
      <div style={{position:'relative'}}>
        ...
        <div style={{display:'flex',gap:20,overflowX:'auto',padding:'4px 32px 20px',scrollbarWidth:'none',scrollSnapType:'x mandatory'}}>
          {pageVendors.map(v=>{
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginTop:8}}>
          <div style={{fontSize:'0.8rem'}}>
            {onReq
              ...
              : <span style={{color:M.forest,fontWeight:700}}>{fmt(total)}</span>}
          </div>
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden',background:M.offWhite}}>
      ...
      <div style={{flex:1,overflowY:'auto',background:M.offWhite,padding:'14px 16px'}}>
        ...
        {tab==='pricing'&&(
          <div style={{background:M.white,borderRadius:12,padding:'14px',border:`1px solid ${M.border}`}}>
            {onReq?(
              ...
              <div>
                {[['Base rate',fmt(vendor.fixed_rate)],[`Travel (${vendor.distance_km||0} km)`,fmt(travel)],...(overnight>0?[['Overnight fee',fmt(overnight)]]:[])]
                  .map(([l,v],i)=>(
        ...
        {tab==='availability'&&(
          <div style={{background:M.white,borderRadius:12,padding:'14px',border:`1px solid ${M.border}`}}>
            <div style={{fontSize:'0.84rem',color:M.body,marginBottom:12}}>
              Check availability for this vendor below.</div>
            <Calendar year={new Date().getFullYear()} month={new Date().getMonth()}
              unavailDates={new Set((vendor.unavail_dates||[]).map(d=>d.date))}
              weddingDate={dateFrom} onPrev={()=>{}} onNext={()=>{}}/>
          </div>
        )}
        <div style={{height:80}}/>
      </div>
...
function MobileFavouritesScreen({user, onOpen, onQuote, dateFrom, onLogin}) {
  ...
  useEffect(()=>{
    ...
    supaFetch(`favourites?customer_id=eq.${user.customerId}&select=vendor:vendors(*,images:vendor_images(*),unavail_dates:vendor_unavailable_dates(date))`)
      .then(data=>{
