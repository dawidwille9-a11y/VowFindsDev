...
export default function SubscriptionPanel({ userId, userEmail, adminOverride=false }) {
  ...
  const isExpired = sub?.status==='cancelled'||sub?.status==='expired'||sub?.status==='past_due';
    ...
    <div style={{display:'flex',flexDirection:'column',gap:14}}>
      ...
      {adminOverride&&(
        <div style={{background:'rgba(201,169,110,0.12)',border:'1px solid var(--gold)',borderRadius:10,padding:'10px 16px',fontSize:'0.8rem',color:'var(--forest)',display:'flex',alignItems:'center',gap:8}}>
          {IC.check(14,'var(--forest)')} <strong>Admin view</strong> — subscription paywall bypassed for this vendor.
        </div>
      )}
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <div>
            ...
            <div style={{fontSize:'0.74rem',color:'var(--mid)',marginTop:2}}>
              {isTrial&&trialEnd&&`Free trial — ${daysLeft} day${daysLeft!==1?'s':''} remaining (ends ${trialEnd.toLocaleDateString('en-ZA',{day:'numeric',month:'long',year:'numeric'})})`}
              {isActive&&periodEnd&&`Next billing date: ${periodEnd.toLocaleDateString('en-ZA',{day:'numeric',month:'long',year:'numeric'})}`}
              {isExpired&&'Your listing is currently hidden from customers.'}
            </div>
          </div>
        </div>
        {sub&&<button onClick={loadHistory} style={{fontSize:'0.73rem',color:'var(--mid)',background:'none',border:'none',cursor:'pointer',textDecoration:'underline'}}>Payment history</button>}
      </div>

      {/* Warning if trial running out */}
      {isTrial&&daysLeft!==null&&daysLeft<=14&&(
        <div style={{background:'rgba(196,130,106,0.08)',border:'1px solid rgba(196,130,106,0.25)',borderRadius:10,padding:'11px 15px',fontSize:'0.83rem',color:'var(--charcoal)',lineHeight:1.6}}>
          ⚠️ Your free trial ends in <strong>{daysLeft} day{daysLeft!==1?'s':''}</strong>. Subscribe below to keep your listing visible to couples.
        </div>
      )}
              ...
              <ul style={{margin:0,padding:0,listStyle:'none',display:'flex',flexDirection:'column',gap:6}}>
                {FEATURES.map(f=>(
                  <li key={f} style={{fontSize:'0.79rem',color:'var(--mid)',display:'flex',alignItems:'center',gap:7}}>
                    <span style={{color:'var(--forest)',fontSize:'0.8rem',flexShrink:0}}>✓</span>{f}
                  </li>
                ))}
              </ul>
      ...
      {isActive&&!sub?.cancel_at_period_end&&(
        <div style={{background:'var(--cream)',borderRadius:10,padding:'13px 16px',fontSize:'0.81rem',color:'var(--mid)',lineHeight:1.7}}>
          ...
          To change your plan or cancel, email{' '}
      ...
      <div style={{fontSize:'0.71rem',color:'var(--light)',textAlign:'center',lineHeight:1.7}}>
        Payments processed securely by PayFast. VowFinds does not store card details.
      </div>
    </div>
  );
}
