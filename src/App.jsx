  ...
  return(
    <div style={{display:'flex',flexDirection:'column',height:'calc(100vh - 56px)',background:'var(--white)',overflow:'hidden',borderRadius:0}}>
      ...
      <div style={{padding:'12px 16px',borderBottom:'1px solid var(--parchment)',display:'flex',alignItems:'center',gap:10,background:'var(--white)',boxShadow:'0 1px 6px rgba(0,0,0,0.05)',flexShrink:0,zIndex:2}}>
        <button onClick={onBack} style={{background:'none',border:'none',cursor:'pointer',color:'var(--mid)',padding:'6px',borderRadius:8,fontSize:'1.1rem',flexShrink:0,display:'flex',alignItems:'center'}}>‹</button>
        {/* Avatar */}
      ...
      <div style={{padding:'10px 12px',borderTop:'1px solid var(--parchment)',background:'var(--white)',flexShrink:0}}>
        <div style={{display:'flex',gap:8,alignItems:'flex-end'}}>
          ...
          <button onClick={()=>fileRef.current?.click()}
            style={{background:'none',border:'none',borderRadius:8,padding:'8px',cursor:'pointer',fontSize:'1.1rem',color:'var(--mid)',flexShrink:0,lineHeight:1,alignSelf:'flex-end'}}>
            📎
          </button>
          {/* Send */}
...
function MobileVendorCard({vendor,onOpen,onQuote,customerId}) {
  ...
  const onReq=isOnRequest(vendor);
  return(
    <div onClick={onOpen} style={{background:'#fff',borderRadius:16,marginBottom:12,overflow:'hidden',boxShadow:'0 2px 12px rgba(0,0,0,0.07)',display:'flex',cursor:'pointer',minHeight:110}}>
      {/* Image */}
      <div style={{width:110,flexShrink:0,background:img?`url(${img}) center/cover`:`linear-gradient(140deg,${vendor.color||'#c8a87a'}cc,${vendor.color||'#c8a87a'}55)`,position:'relative'}}>
        <div style={{position:'absolute',bottom:6,left:6,background:'rgba(58,74,63,0.85)',color:'var(--gold-light)',fontSize:'0.55rem',letterSpacing:'0.1em',textTransform:'uppercase',padding:'2px 7px',borderRadius:999}}>{TYPE_EMOJI[vendor.type]} {vendor.type}</div>
      </div>
      {/* Content */}
      <div style={{flex:1,padding:'12px 14px',display:'flex',flexDirection:'column',justifyContent:'space-between',minWidth:0}}>
        <div>
          ...
          <div style={{fontSize:'0.72rem',color:'var(--light)',marginBottom:6}}>📍 {vendor.location}{vendor.distance_km?` · ${vendor.distance_km} km`:''}</div>
...
function MobileSearchScreen({onVendorsLoaded,initialVenue='',initialLL=null}) {
  ...
  return(
    <div style={{flex:1,overflowY:'auto',background:'var(--cream)'}}>
      {/* Hero */}
      <div style={{background:'linear-gradient(160deg,var(--forest) 0%,#1e2820 100%)',padding:'40px 20px 32px',position:'relative',overflow:'hidden'}}>
        <div style={{position:'absolute',top:-60,right:-60,width:200,height:200,borderRadius:'50%',background:'rgba(255,255,255,0.03)'}}/>
        <div style={{position:'absolute',bottom:-40,left:-40,width:160,height:160,borderRadius:'50%',background:'rgba(201,169,110,0.06)'}}/>
        <div style={{fontSize:'0.66rem',letterSpacing:'0.22em',textTransform:'uppercase',color:'var(--gold)',marginBottom:8,position:'relative',zIndex:1}}>Your wedding, your way</div>
        <h1 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.9rem',fontWeight:300,color:'var(--cream)',lineHeight:1.2,marginBottom:6,position:'relative',zIndex:1}}>Find the <em style={{color:'var(--blush)',fontStyle:'italic'}}>perfect</em> vendors</h1>
        <p style={{fontSize:'0.82rem',color:'rgba(250,246,241,0.5)',lineHeight:1.6,position:'relative',zIndex:1}}>Enter your venue and we'll calculate real travel costs for every vendor.</p>
      </div>

      {/* Search card */}
      <div style={{margin:'0 16px',marginTop:-20,background:'#fff',borderRadius:20,padding:'20px',boxShadow:'0 8px 40px rgba(0,0,0,0.15)',position:'relative',zIndex:2}}>
        {/* Venue */}
        <div style={{marginBottom:14}}>
...
function MobileResultsScreen({vendors,venue,venueLL,dateFrom,onBack,onOpen,onQuote,customerId,activeType,setActiveType}) {
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      {/* Header */}
      <div style={{background:'var(--forest)',padding:'12px 16px',display:'flex',alignItems:'center',gap:10,flexShrink:0}}>
        <button onClick={onBack} style={{background:'rgba(255,255,255,0.12)',border:'none',borderRadius:8,padding:'6px 10px',color:'rgba(255,255,255,0.8)',cursor:'pointer',fontSize:'0.8rem',flexShrink:0}}>‹ Search</button>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:'0.78rem',fontWeight:600,color:'var(--gold-light)',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>📍 {venue||'All vendors'}</div>
          <div style={{fontSize:'0.68rem',color:'rgba(255,255,255,0.5)'}}>{vendors.length} vendors found{dateFrom?` · ${formatDateDisplay(dateFrom)}`:''}</div>
        </div>
      </div>
      ...
      <div style={{background:'#fff',padding:'10px 14px',borderBottom:'1px solid var(--parchment)',flexShrink:0}}>
        <div style={{background:'var(--cream)',borderRadius:10,padding:'8px 14px',display:'flex',alignItems:'center',gap:8}}>
          <span style={{fontSize:'0.9rem',color:'var(--light)'}}>🔍</span>
          ...
          {search&&<button onClick={()=>setSearch('')} style={{background:'none',border:'none',color:'var(--light)',cursor:'pointer',fontSize:'0.9rem',padding:0}}>✕</button>}
        </div>
      </div>
      ...
      {/* Vendor list */}
      <div style={{flex:1,overflowY:'auto',padding:'12px 14px',background:'#f7f3ee'}}>
        {filtered.length===0
...
function MobileVendorDetail({vendor,dateFrom,venueLabel,venueLatLng,onBack,onQuote}) {
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      ...
      {/* Tab content */}
      <div style={{flex:1,overflowY:'auto',background:'#f7f3ee',padding:'14px 16px'}}>
        {tab==='about'&&(
          <div>
            ...
            {vendor.instagram&&(
              <a href={`https://instagram.com/${vendor.instagram.replace('@','')}`} target="_blank" rel="noreferrer" style={{display:'flex',alignItems:'center',gap:10,background:'#fff',borderRadius:12,padding:'12px 14px',textDecoration:'none',marginBottom:12}}>
                <div style={{width:32,height:32,borderRadius:8,background:'linear-gradient(135deg,#f09433,#e6683c,#dc2743,#cc2366,#bc1888)',display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontSize:'0.9rem',flexShrink:0}}>ig</div>
                <div>
...
function MobileQuotesScreen({user,onBrowse,initialLead=null,onRequestQuote}) {
  ...
  return(
    <div style={{flex:1,display:'flex',flexDirection:'column',overflow:'hidden'}}>
      {/* Header */}
      <div style={{background:'var(--forest)',padding:'16px 16px 14px',flexShrink:0}}>
      ...
      <div style={{flex:1,overflowY:'auto',background:'#f7f3ee',padding:'12px 14px'}}>
        {loading?(
          ...
          orderedTypes.map(type=>(
            <div key={type} style={{marginBottom:22}}>
              ...
              {grouped[type].map(lead=>{
                ...
                return(
                  <div key={lead.id} onClick={()=>setActiveLead(lead)}
                    style={{background:'#fff',borderRadius:12,padding:'12px 14px',marginBottom:8,cursor:'pointer',display:'flex',alignItems:'center',gap:12,borderLeft:`3px solid ${hasNew?'var(--rose)':'var(--parchment)'}`}}>
...
function MobileApp({user,onLogin,onLogout,requestQuote,quoteVendor,setQuoteVendor,newLeadAfterQuote,setNewLeadAfterQuote}) {
  // Tab states: 'home' | 'browse' | 'quotes' | 'plan'
  ...
  const [showSearch,setShowSearch]=useState(true); // home tab: search vs results
  const [showLoginModal,setShowLoginModal]=useState(false);
  const [showCustomerAuth,setShowCustomerAuth]=useState(false);

  // If new lead after quote → go to quotes tab
  useEffect(()=>{
    if(newLeadAfterQuote){setTab('quotes');}
  },[newLeadAfterQuote]);
  ...
  const unreadCount=0; // could wire up later

  // Full-screen vendor detail overlay
  if(activeVendor) return(
    <div style={{position:'fixed',inset:0,zIndex:600,background:'#f7f3ee',display:'flex',flexDirection:'column'}}>
      <MobileVendorDetail
        vendor={activeVendor}
        dateFrom={dateFrom}
        venueLabel={venue}
        venueLatLng={venueLL}
        onBack={()=>setActiveVendor(null)}
        onQuote={()=>{handleRequestQuote(activeVendor);setActiveVendor(null);}}
      />
    </div>
  );

  return(
    <div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',background:'#f7f3ee',fontFamily:"'DM Sans',sans-serif"}}>
      <GlobalStyles/>
      ...
      {showCustomerAuth&&<CustomerAuthModal onLogin={u=>{onLogin(u);setShowCustomerAuth(false);}} onClose={()=>setShowCustomerAuth(false)}/>}

      {/* Top status bar area — just brand + auth */}
      <div style={{background:'var(--forest)',padding:'10px 16px 8px',display:'flex',alignItems:'center',justifyContent:'space-between',flexShrink:0,paddingTop:'max(10px, env(safe-area-inset-top))'}}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.2rem',fontWeight:300,color:'var(--gold-light)',letterSpacing:'0.06em'}}>Vow<span style={{color:'var(--blush)',fontStyle:'italic'}}>Finds</span></div>
        {user&&user.role==='customer'?(
          <div style={{display:'flex',alignItems:'center',gap:8}}>
            <span style={{fontSize:'0.72rem',color:'rgba(255,255,255,0.5)'}}>Hi, {user.name}</span>
            <button onClick={onLogout} style={{background:'rgba(255,255,255,0.1)',border:'none',borderRadius:7,padding:'4px 10px',color:'rgba(255,255,255,0.7)',cursor:'pointer',fontSize:'0.72rem'}}>Logout</button>
          </div>
        ):(
          <div style={{display:'flex',gap:6}}>
            <button onClick={()=>setShowCustomerAuth(true)} style={{background:'none',border:'1px solid rgba(255,255,255,0.2)',borderRadius:7,padding:'4px 10px',color:'rgba(255,255,255,0.65)',cursor:'pointer',fontSize:'0.72rem'}}>Login</button>
            <button onClick={()=>setShowCustomerAuth(true)} style={{background:'rgba(201,169,110,0.2)',border:'1px solid rgba(201,169,110,0.35)',borderRadius:7,padding:'4px 10px',color:'var(--gold-light)',cursor:'pointer',fontSize:'0.72rem'}}>Sign Up</button>
          </div>
        )}
      </div>
      ...
      <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
        {tab==='home'&&(
          showSearch
            ...
            :<MobileResultsScreen vendors={vendors} venue={venue} venueLL={venueLL} dateFrom={dateFrom} onBack={()=>setShowSearch(true)} onOpen={setActiveVendor} onQuote={handleRequestQuote} customerId={user?.customerId} activeType={activeType} setActiveType={setActiveType}/>
        )}
        {tab==='browse'&&(
          vendors.length>0
            ?<MobileResultsScreen vendors={vendors} venue={venue} venueLL={venueLL} dateFrom={dateFrom} onBack={()=>{setTab('home');setShowSearch(true);}} onOpen={setActiveVendor} onQuote={handleRequestQuote} customerId={user?.customerId} activeType={activeType} setActiveType={setActiveType}/>
            :<MobileSearchScreen onVendorsLoaded={handleVendorsLoaded}/>
        )}
        {tab==='quotes'&&(
          user&&user.role==='customer'
            ?<MobileQuotesScreen user={user} onBrowse={()=>{setTab('browse');}} initialLead={newLeadAfterQuote} onRequestQuote={handleRequestQuote}/>
            :<div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'32px 24px',gap:16,background:'#f7f3ee'}}>
              <div style={{fontSize:'3rem'}}>💬</div>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.4rem',color:'var(--forest)',textAlign:'center'}}>See your quotes here</div>
              <p style={{fontSize:'0.84rem',color:'var(--mid)',textAlign:'center',lineHeight:1.6}}>Log in to view your quote requests and chat with vendors.</p>
              <button onClick={()=>setShowCustomerAuth(true)} style={{background:'var(--forest)',color:'var(--gold-light)',border:'none',borderRadius:12,padding:'13px 32px',fontSize:'0.92rem',fontWeight:600,cursor:'pointer'}}>Log In</button>
            </div>
        )}
        {tab==='plan'&&(
          <div style={{flex:1,overflow:'hidden',display:'flex',flexDirection:'column'}}>
            <WeddingPlan vendors={vendors} onClose={()=>setTab('home')}
              onSearchVendors={({venueName,venueLL:ll,type,maxPrice})=>{
      ...
      {/* Bottom tab bar */}
      <div style={{background:'#fff',borderTop:'1px solid var(--parchment)',display:'flex',flexShrink:0,paddingBottom:'max(8px, env(safe-area-inset-bottom))',boxShadow:'0 -2px 12px rgba(0,0,0,0.06)'}}>
        {TAB_CONFIG.map(t=>{
          const isActive=tab===t.id;
          const badge=t.id==='quotes'&&unreadCount>0?unreadCount:null;
          return(
            <button key={t.id} onClick={()=>setTab(t.id)}
              style={{flex:1,background:'none',border:'none',padding:'10px 4px 6px',cursor:'pointer',display:'flex',flexDirection:'column',alignItems:'center',gap:3,position:'relative'}}>
              <div style={{fontSize:'1.3rem',lineHeight:1,filter:isActive?'none':'grayscale(0.3) opacity(0.6)'}}>{t.emoji}</div>
              <div style={{fontSize:'0.62rem',fontWeight:isActive?700:400,color:isActive?'var(--forest)':'var(--light)',letterSpacing:'0.02em'}}>{t.label}</div>
              {isActive&&<div style={{position:'absolute',top:0,left:'50%',transform:'translateX(-50%)',width:24,height:2.5,background:'var(--forest)',borderRadius:999}}/>}
              {badge&&<div style={{position:'absolute',top:6,right:'50%',marginRight:-14,background:'var(--rose)',color:'#fff',borderRadius:999,fontSize:'0.55rem',fontWeight:700,padding:'1px 5px',minWidth:14,textAlign:'center'}}>{badge}</div>}
            </button>
          );
        })}
      </div>
