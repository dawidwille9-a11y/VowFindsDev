...
import VendorLane from './components/vendor/VendorLane.jsx';
...
export default function VowFinds() {
  ...
  // Show legal page when requested
  if(showLegal) return <LegalPage initialTab={showLegal} onClose={()=>setShowLegal(null)}/>;
  ...
  const isEmailVerification = window.location.hash.includes('access_token') &&
    (window.location.hash.includes('type=signup') || window.location.hash.includes('type=recovery'));
    ...
    <>
      ...
      <div style={{background:'var(--forest)',padding:'28px 32px',textAlign:'center',marginTop:40}}>
        ...
        <p style={{fontSize:'0.68rem',color:'rgba(255,255,255,0.25)',marginTop:10,marginBottom:0}}>
          {'© '}{new Date().getFullYear()}{' VowFinds · vowfinds.co.za'}
        </p>
      </div>
    </>
  );
