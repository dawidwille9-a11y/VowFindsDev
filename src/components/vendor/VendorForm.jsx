...
import { supaFetch, storageUrl, loadGoogleMaps } from '../../api.js';
...
const sectionStyle = {background:'var(--white)',borderRadius:14,padding:'24px 28px',marginBottom:20,boxShadow:'var(--card-shadow)'};
const h3Style = {fontFamily:"'Cormorant Garamond',serif",fontSize:'1.2rem',color:'var(--forest)',marginBottom:18,paddingBottom:10,borderBottom:'1px solid var(--parchment)',fontWeight:600};

function VendorForm({initialData=null, vendorId=null, userId=null, token=null, onSaved, onCancel}) {
  ...
  const fileRef=useRef(),locRef=useRef(),displayPicRef=useRef();
  const [displayPic,setDisplayPic]=useState(initialData?.display_picture||'');
  ...
  async function addPackage(){
    ...
    const res=await supaFetch('vendor_packages',{method:'POST',body:JSON.stringify(payload),prefer:'return=representation',headers:getTokenHeaders()});
  ...
  async function saveEditPackage(pkg){
    ...
    await supaFetch(`vendor_packages?id=eq.${pkg.id}`,{method:'PATCH',body:JSON.stringify(payload),prefer:'return=minimal',headers:getTokenHeaders()});
    setPackages(prev=>prev.map(p=>p.id===pkg.id?{...p,...payload}:p));
    setEditingPkg(null);
  }

  async function deletePackage(id){
    await supaFetch(`vendor_packages?id=eq.${id}`,{method:'DELETE',prefer:'return=minimal',headers:getTokenHeaders()});
    setPackages(prev=>prev.filter(p=>p.id!==id));
  }

  // Get auth token from session storage (set on login)
  ...
  async function uploadImages(files){
    ...
    for(const file of files){
      try{
        // Compress before uploading — reduces 4MB phone photos to ~300KB
        const compressed = await imageCompression(file, {
          maxSizeMB: 0.4,          // target 400KB max
          maxWidthOrHeight: 1400,  // enough for web display, no larger
          useWebWorker: true,
          fileType: 'image/webp',  // webp is smaller than jpeg with same quality
        });
        const ext = 'webp';
        const path = `${currentVendorId}/${Date.now()}.${ext}`;
        const ur = await fetch(`${SUPABASE_URL}/storage/v1/object/vendor-images/${path}`,{
          method:'POST',
          headers:{'apikey':SUPABASE_PUB_KEY,'Authorization':`Bearer ${SUPABASE_ANON_KEY}`,'Content-Type':'image/webp'},
          body: compressed,
        });
        if(ur.ok){
        ...
        setSaveError('Could not process image: '+compErr.message);
      }
    }
    setUploading(false);
  }

  async function uploadDisplayPic(file) {
    if (!currentVendorId) { setSaveError('Save profile first, then upload display picture.'); return; }
    setUploadingDP(true);
    try {
      const compressed = await imageCompression(file, { maxSizeMB:0.3, maxWidthOrHeight:600, useWebWorker:true, fileType:'image/webp' });
      const path = `${currentVendorId}/display_${Date.now()}.webp`;
      const ur = await fetch(`${SUPABASE_URL}/storage/v1/object/vendor-images/${path}`, {
        method:'POST', headers:{'apikey':SUPABASE_PUB_KEY,'Authorization':`Bearer ${SUPABASE_ANON_KEY}`,'Content-Type':'image/webp'}, body:compressed,
      });
      if (ur.ok) {
        const url = storageUrl(path);
        setDisplayPic(url);
        // Save immediately to vendor record
        await supaFetch(`vendors?id=eq.${currentVendorId}`, {method:'PATCH', body:JSON.stringify({display_picture:url}), prefer:'return=minimal', headers:getTokenHeaders()});
      } else { setSaveError('Display picture upload failed.'); }
    } catch(e) { setSaveError('Upload error: '+e.message); }
  ...
  return (
    <div>
      ...
      {/* ── Display Picture ── */}
      <div style={sectionStyle}>
        <h3 style={h3Style}>Display Picture</h3>
        <p style={{fontSize:'0.82rem',color:'var(--mid)',marginBottom:16,lineHeight:1.6}}>
          This is the photo shown on your vendor card in search results. Use a high-quality image that best represents your work — ideally square or portrait format.
        </p>
        <div style={{display:'flex',alignItems:'center',gap:20}}>
          {/* Current display pic preview */}
          <div
            onClick={()=>displayPicRef.current?.click()}
            style={{width:110,height:110,borderRadius:14,flexShrink:0,cursor:'pointer',
              background:displayPic?`url(${displayPic}) center/cover`:'var(--parchment)',
              border:`2px dashed ${displayPic?'var(--forest)':'var(--blush)'}`,
              display:'flex',alignItems:'center',justifyContent:'center',
              overflow:'hidden',position:'relative'}}>
            {!displayPic&&<div style={{textAlign:'center',color:'var(--light)'}}>
              <div style={{fontSize:'1.8rem',marginBottom:4}}>📷</div>
              <div style={{fontSize:'0.68rem'}}>Upload photo</div>
            </div>}
            {uploadingDP&&<div style={{position:'absolute',inset:0,background:'rgba(0,0,0,0.4)',display:'flex',alignItems:'center',justifyContent:'center',color:'white',fontSize:'0.8rem'}}>Uploading…</div>}
          </div>
          <div>
            <button type="button" onClick={()=>displayPicRef.current?.click()}
              style={{background:'var(--forest)',color:'var(--gold-light)',border:'none',borderRadius:8,padding:'9px 18px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.82rem',fontWeight:500,cursor:'pointer',display:'block',marginBottom:8}}>
              {displayPic?'Change Display Picture':'Upload Display Picture'}
            </button>
            {displayPic&&<button type="button" onClick={async()=>{
                setDisplayPic('');
                if(currentVendorId){
                  await supaFetch(`vendors?id=eq.${currentVendorId}`,{method:'PATCH',body:JSON.stringify({display_picture:null}),prefer:'return=minimal',headers:getTokenHeaders()}).catch(()=>{});
                }
              }}
              style={{background:'#fce8e4',color:'#b85a45',border:'none',borderRadius:8,padding:'7px 14px',fontSize:'0.78rem',cursor:'pointer'}}>
              Remove
            </button>}
            <p style={{fontSize:'0.72rem',color:'var(--light)',marginTop:8}}>Save profile first before uploading.</p>
          </div>
        </div>
        <input ref={displayPicRef} type="file" accept="image/*" style={{display:'none'}} onChange={e=>{if(e.target.files[0])uploadDisplayPic(e.target.files[0]);}}/>
      </div>
