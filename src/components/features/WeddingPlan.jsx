...
import { ALL_TYPES, ON_REQUEST_TYPES, BOOKING_ORDER, ADDITIONAL_VENDORS, BUDGET_RATIOS } from '../../constants.js';
...
function WeddingPlan({onClose, vendors: passedVendors, onSearchVendors, hideHeader=false}) {
  ...
  // Recommended spend — ratios applied to total budget (venue-inclusive)
  ...
  // Venue is 20% of total — shown first, then vendor budget is the remaining 80%
  const venueAmount = recommendedSpend['Venue'] || Math.round(budget * 0.20);
  const vendorBudget = budget - venueAmount;

  // Leftover vendor budget after all primary categories
  const primaryTotal = useMemo(()=>{
    if(!budget) return 0;
    return BOOKING_ORDER.reduce((sum,o)=>sum+(recommendedSpend[o.type]||0),0);
  },[recommendedSpend,budget]);
  const additionalTotal = ADDITIONAL_VENDORS.reduce((sum,a)=>sum+(recommendedSpend[a.type]||0),0);
  const leftover = budget - venueAmount - primaryTotal - additionalTotal;
  ...
  if (planStep === 'intro') return (
    <div style={{minHeight:'100vh',background:'var(--cream)'}}>
      {!hideHeader&&<div style={{background:'var(--white)',borderBottom:'1px solid var(--parchment)',padding:'14px 24px',display:'flex',alignItems:'center',gap:12,position:'sticky',top:0,zIndex:10}}>
        ...
        <div style={{background:'var(--white)',borderRadius:16,boxShadow:'var(--card-shadow)',padding:'28px'}}>
          ...
          {budget>0&&(
            <div style={{background:'rgba(58,74,63,0.05)',borderRadius:10,padding:'14px 16px',marginBottom:20,border:'1px solid rgba(58,74,63,0.1)'}}>
              ...
              {/* Venue first */}
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:'0.8rem',padding:'5px 0',borderBottom:'1px solid rgba(58,74,63,0.06)',background:'rgba(201,169,110,0.08)',margin:'0 -4px',padding:'5px 4px',borderRadius:4}}>
                <span style={{color:'var(--charcoal)',fontWeight:600,display:'flex',alignItems:'center',gap:5}}>{IC.pin(13,'var(--gold)')} Venue</span>
                <span style={{fontWeight:700,color:'var(--gold)'}}>{fmt(Math.round(budget*0.20))}</span>
              </div>
              {BOOKING_ORDER.map(({type})=>(
                <div key={type} style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:'0.8rem',padding:'4px 0',borderBottom:'1px solid rgba(58,74,63,0.06)'}}>
                  ...
                  <span style={{fontWeight:600,color:'var(--forest)'}}>{fmt(Math.round(budget*(BUDGET_RATIOS[type]||0.01)))}</span>
                </div>
              ))}
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',fontSize:'0.8rem',padding:'6px 0 0',marginTop:4,borderTop:'1px solid rgba(58,74,63,0.12)'}}>
                <span style={{color:'var(--mid)',fontStyle:'italic'}}>+ Additional (Barista, etc.)</span>
                <span style={{fontWeight:600,color:'var(--gold)'}}>{fmt(Math.max(0,leftover))}</span>
              </div>
            </div>
          )}
  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--cream)'}}>
      {!hideHeader&&<div style={{background:'var(--white)',borderBottom:'1px solid var(--parchment)',padding:'14px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:10}}>
      ...
      <div style={{maxWidth:860,margin:'0 auto',padding:'24px 24px 60px'}}>
        ...
        {/* ── VENUE CARD — always first ── */}
        {budget>0&&(
          <div style={{...cardStyle,border:'2px solid var(--gold)'}}>
            <div style={{background:'linear-gradient(135deg,#2c3e2d,#3a5a3f)',padding:'14px 20px',display:'flex',alignItems:'center',gap:12}}>
              <div style={{width:36,height:36,borderRadius:'50%',background:'rgba(201,169,110,0.2)',display:'flex',alignItems:'center',justifyContent:'center',flexShrink:0}}>
                {IC.pin(20,'var(--gold-light)')}
              </div>
              <div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.2rem',color:'var(--gold-light)',fontWeight:600}}>Venue</div>
                <div style={{fontSize:'0.72rem',color:'rgba(232,213,163,0.7)',marginTop:1}}>Book first — your venue determines your date</div>
              </div>
              <div style={{marginLeft:'auto',background:'rgba(201,169,110,0.15)',borderRadius:10,padding:'8px 14px',textAlign:'right'}}>
                <div style={{fontSize:'0.62rem',color:'rgba(232,213,163,0.7)',letterSpacing:'0.1em',textTransform:'uppercase'}}>Recommended</div>
                <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.5rem',color:'var(--gold-light)',fontWeight:600}}>{fmt(venueAmount)}</div>
                <div style={{fontSize:'0.65rem',color:'rgba(232,213,163,0.6)'}}>20% of total budget</div>
              </div>
            </div>
            <div style={{padding:'18px 20px'}}>
              <div style={{fontSize:'0.88rem',color:'var(--charcoal)',lineHeight:1.65,marginBottom:8}}>
                Your venue is the foundation of your wedding — it sets the guest count, atmosphere, and date. Most other vendors are chosen around your venue.
              </div>
              <div style={{fontSize:'0.8rem',color:'var(--mid)',background:'var(--parchment)',borderRadius:8,padding:'8px 12px',lineHeight:1.55,marginBottom:14}}>
                <strong style={{color:'var(--forest)'}}>Why book first:</strong> Venues fill up 12–18 months in advance for peak dates. Book your venue before locking in any other vendor.
              </div>
              <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
                <div style={{background:'var(--parchment)',borderRadius:10,padding:'14px 16px'}}>
                  <div style={{fontSize:'0.68rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--mid)',marginBottom:6}}>Your total budget</div>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.4rem',fontWeight:600,color:'var(--forest)'}}>{fmt(budget)}</div>
                </div>
                <div style={{background:'rgba(58,74,63,0.06)',borderRadius:10,padding:'14px 16px',border:'1px solid rgba(58,74,63,0.1)'}}>
                  <div style={{fontSize:'0.68rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--mid)',marginBottom:6}}>Remaining for vendors</div>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.4rem',fontWeight:600,color:'var(--rose)'}}>{fmt(budget-venueAmount)}</div>
                </div>
              </div>
              {weddingVenue&&(
                <div style={{marginTop:12,display:'flex',alignItems:'center',gap:8,background:'rgba(58,74,63,0.06)',borderRadius:8,padding:'10px 14px'}}>
                  {IC.pin(14,'var(--forest)')}
                  <span style={{fontSize:'0.84rem',color:'var(--forest)',fontWeight:500}}>{weddingVenue}</span>
                  <span style={{fontSize:'0.74rem',color:'var(--mid)',marginLeft:4}}>· your selected venue</span>
                </div>
              )}
            </div>
          </div>
