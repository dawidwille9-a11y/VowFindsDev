...
function FavouritesView({customerId,onOpenDetail,onRequestQuote,dateFrom,dateTo,venueLatLng=null,venue=''}) {
        ...
        <div key={type} style={{marginBottom:32}}>
          ...
          <div style={{display:'flex',gap:18,overflowX:'auto',paddingBottom:8}}>
            {vv.map(v=>{
              ...
              return<VendorCard key={v.id} vendor={v} unavail={unavail} dateFrom={dateFrom} dateTo={dateTo} onClick={()=>onOpenDetail(v,venueLatLng,venue)} onRequestQuote={()=>onRequestQuote&&onRequestQuote(v)} customerId={customerId} onFav={!!customerId} onFavRemoved={()=>setFavVendors(prev=>prev.filter(fv=>fv.id!==v.id))}/>;
            })}
          </div>
        </div>
      ))}
