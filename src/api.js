...
async function getAuthToken() {
  try {
    // First try the Supabase SDK session (set by signInWithPassword)
    const { data } = await supabase.auth.getSession();
    if (data?.session?.access_token) return data.session.access_token;
  } catch(e) {}
  try {
    // Fallback: check our own session storage for the token saved on login
...
// Returns set of vendor user_ids that have active or trial subscriptions.
// Vendors with cancelled/expired subscriptions are hidden from customers.
export async function getActiveVendorUserIds() {
  try {
    const data = await supaFetch(
      'subscriptions?status=in.(active,trial)&select=vendor_user_id'
    );
    if (!Array.isArray(data)) return null;
    return new Set(data.map(s => s.vendor_user_id));
  } catch(e) {
    return null; // on error show all vendors (safe fallback)
  }
}
