...
import { signIn, signUp, supaFetch } from '../../api.js';
...
function CustomerAuthModal({onLogin, onClose, redirectVendor=null, onVendorLogin=null}) {
  ...
  async function handleLogin(e){
    ...
    try{
      ...
      // Fetch or create customer record using the authenticated session token
      let customers=await supaFetch(`customers?user_id=eq.${data.user.id}&select=*`,{headers:authHeaders});
      let customer=Array.isArray(customers)?customers[0]:customers;
      if(!customer){
        const res=await supaFetch('customers',{method:'POST',body:JSON.stringify({user_id:data.user.id,name:data.user.user_metadata?.name||email.split('@')[0],email:data.user.email}),prefer:'return=representation',headers:authHeaders});
  ...
  async function handleRegister(e){
    ...
    try{
      ...
      // Create customer profile — use session token if available, else it will be created on first login
      if(data.user && data.session){
        await supaFetch('customers',{method:'POST',body:JSON.stringify({user_id:data.user.id,name,email}),prefer:'return=minimal',headers:{'Authorization':`Bearer ${data.session.access_token}`}});
      } else if(data.user){
        // Store name in user metadata so we can use it when they verify and log in
