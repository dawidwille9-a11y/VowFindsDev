A// VowFinds – Supabase + Google Maps + Auth
...
const VendorCard=memo(function VendorCard({vendor,unavail,dateFrom,dateTo,onClick,onRequestQuote,customerId=null,onFav=false}) {
  ...
  return (
    <div onClick={onClick} style={{background:'var(--white)',borderRadius:16,overflow:'hidden',boxShadow:'var(--card-shadow)',flex:'0 0 288px',width:288,position:'relative',cursor:'pointer',transition:'box-shadow 0.25s, transform 0.25s',filter:unavail?'saturate(0.3) opacity(0.75)':'none'}}
      ...
      <div style={{height:160,position:'relative',background:primaryImg?`url(${primaryImg}) center/cover`:`linear-gradient(140deg,${vendor.color||'#c8a87a'}ee 0%,${vendor.color||'#c8a87a'}66 100%),linear-gradient(160deg,#ede5db 0%,#d8ccc0 100%)`}}>
        <div style={{position:'absolute',top:10,left:10,background:'rgba(58,74,63,0.88)',color:'var(--gold-light)',fontSize:'0.65rem',letterSpacing:'0.12em',textTransform:'uppercase',padding:'3px 9px',borderRadius:999,backdropFilter:'blur(4px)'}}>{vendor.type}</div>
      ...
      <div style={{padding:'16px 18px 18px'}}>
        <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.15rem',fontWeight:600,color:'var(--forest)',marginBottom:3}}>{vendor.name}</div>
        <div style={{fontSize:'0.75rem',color:'var(--light)',marginBottom:10}}>📍 {vendor.location}{vendor.distance_km?` · ${vendor.distance_km} km away`:''}</div>
        ...
        {isOnRequest(vendor)?(
          <div style={{background:'var(--cream)',borderRadius:9,padding:'10px 12px',display:'flex',flexDirection:'column',gap:4}}>
      ...
      <div style={{background:'linear-gradient(160deg,var(--forest) 0%,#2a3830 60%,#1e2820 100%)',position:'relative',overflow:'hidden',padding:'52px 24px 48px',display:'flex',flexDirection:'column',alignItems:'center'}}>
        ...
        <div style={{background:'var(--white)',borderRadius:20,padding:'28px 32px',width:'100%',maxWidth:780,boxShadow:'0 16px 60px rgba(0,0,0,0.28)',position:'relative',zIndex:2,marginBottom:16}}>
          <div style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr',gap:14,marginBottom:16,alignItems:'end'}}>
            ...
            <div>
              <label style={{fontSize:'0.68rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--mid)',marginBottom:5,display:'block'}}>From Date</label>
              <input type="date" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} style={{border:'1.5px solid var(--parchment)',borderRadius:9,padding:'11px 14px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.9rem',color:'var(--charcoal)',background:'var(--cream)',outline:'none',width:'100%'}}/>
            </div>
            <div>
              <label style={{fontSize:'0.68rem',letterSpacing:'0.1em',textTransform:'uppercase',color:'var(--mid)',marginBottom:5,display:'block'}}>To Date</label>
              <input type="date" value={dateTo} onChange={e=>setDateTo(e.target.value)} min={dateFrom} style={{border:'1.5px solid var(--parchment)',borderRadius:9,padding:'11px 14px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.9rem',color:'var(--charcoal)',background:'var(--cream)',outline:'none',width:'100%'}}/>
            </div>
          </div>
          {dateFrom&&dateTo&&dateFrom===dateTo&&<div style={{fontSize:'0.75rem',color:'var(--gold)',marginBottom:14,padding:'6px 12px',background:'rgba(201,169,110,0.1)',borderRadius:7,display:'inline-block'}}>📅 Single day: {formatDateDisplay(dateFrom)}</div>}
          {dateFrom&&dateTo&&dateFrom<dateTo&&<div style={{fontSize:'0.75rem',color:'var(--forest)',marginBottom:14,padding:'6px 12px',background:'rgba(58,74,63,0.07)',borderRadius:7,display:'inline-block'}}>📅 {formatDateDisplay(dateFrom)} → {formatDateDisplay(dateTo)}</div>}
          ...
          <div style={{
            ...
            <div style={{background:'var(--white)',borderRadius:20,padding:'28px 32px',width:'100%',maxWidth:780,boxShadow:'0 16px 60px rgba(0,0,0,0.28)',position:'relative',zIndex:2,marginBottom:16}}>
              ...
              {dateFrom&&dateTo&&dateFrom===dateTo&&(
                <div style={{fontSize:'0.75rem',color:'var(--gold)',marginBottom:14,padding:'6px 12px',background:'rgba(201,169,110,0.1)',borderRadius:7,display:'inline-block'}}>📅 Single day: {formatDateDisplay(dateFrom)}</div>
              )}
              {dateFrom&&dateTo&&dateFrom<dateTo&&(
                <div style={{fontSize:'0.75rem',color:'var(--forest)',marginBottom:14,padding:'6px 12px',background:'rgba(58,74,63,0.07)',borderRadius:7,display:'inline-block'}}>📅 {formatDateDisplay(dateFrom)} → {formatDateDisplay(dateTo)}</div>
              )}
