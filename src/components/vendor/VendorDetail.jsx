...
import { fmt, formatDateDisplay, isOnRequest, isVendorUnavailable, calcTotal, calcPackageTotal, cityFromLocation } from '../../utils.js';
    ...
    <div style={{background:'var(--cream)',minHeight:'100vh'}}>
      ...
      <div className="vf-vendor-detail-hero" style={{position:'relative',height:300,overflow:'hidden',marginTop:0,background:(vendor.display_picture||galleryImgs[0]?.url)?`url(${vendor.display_picture||galleryImgs[0]?.url}) center/cover`:`linear-gradient(140deg,${vendor.color||'#c8a87a'}ff,${vendor.color||'#c8a87a'}88)`}}>
        ...
        <div style={{position:'absolute',bottom:0,left:0,right:0,padding:'28px 32px'}}>
          ...
          <div style={{fontSize:'0.82rem',color:'rgba(250,246,241,0.65)',display:'flex',alignItems:'center',gap:5}}>{IC.pin(13,'rgba(250,246,241,0.65)')}{cityFromLocation(vendor.location)}{venueLabel&&vendor.distance_km?` · ${vendor.distance_km} km from ${venueLabel}`:''}</div>
        </div>
      </div>
      <div className="vf-vendor-detail-grid" style={{maxWidth:1040,margin:'0 auto',padding:'32px 32px 60px',display:'grid',gridTemplateColumns:'1fr 340px',gap:36}}>
        <div>
          <section style={{marginBottom:28}}>
              <h3 style={{fontFamily:"'Cormorant Garamond',serif",fontSize:'1.3rem',color:'var(--forest)',marginBottom:14,paddingBottom:10,borderBottom:'1px solid var(--parchment)'}}>About this vendor</h3>
              {vendor.description&&<p style={{fontSize:'0.92rem',color:'var(--mid)',lineHeight:1.85,marginBottom:vendor.extra_info?16:0}}>{vendor.description}</p>}
              {vendor.extra_info&&(
                <div style={{background:'var(--cream)',borderRadius:12,padding:'16px 20px',borderLeft:'3px solid var(--gold)',marginTop:4}}>
                  <div style={{fontSize:'0.68rem',letterSpacing:'0.12em',textTransform:'uppercase',color:'var(--gold)',fontWeight:600,marginBottom:8}}>Additional Information</div>
                  <p style={{fontSize:'0.88rem',color:'var(--mid)',lineHeight:1.85,margin:0}}>{vendor.extra_info}</p>
                </div>
              )}
            </section>
