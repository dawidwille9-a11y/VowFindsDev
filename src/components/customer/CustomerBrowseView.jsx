...
import VendorLane, { VendorGrid } from '../vendor/VendorLane.jsx';
...
function CustomerBrowseView({user,venue,setVenue,venueLatLng,setVenueLatLng,dateFrom,setDateFrom,dateTo,setDateTo,selectedTypes,setSelectedTypes,vendors,setVendors,loading,setLoading,loadError,setLoadError,calcProgress,setCalcProgress,searched,setSearched,showMap,setShowMap,openDetail,onRequestQuote,onOpenScenario,onOpenFavourites,browseView,setBrowseView,planMaxPrices=null,fromPlan=false,onReturnToPlan=null}) {
  ...
  async function loadVendors(latLng){
    ...
    try{
      ...
      let finalVendors=data;
      if(latLng){
        setCalcProgress('Calculating distances…');
        try{
          const kms=await getBatchDistancesKm(latLng,data);
          finalVendors=data.map((v,i)=>({...v,distance_km:kms[i]||0}));
        }catch{finalVendors=data;}
      }
      setCalcProgress('Checking listings…');
      const activeUserIds=await getActiveVendorUserIds();
      if(activeUserIds) finalVendors=finalVendors.filter(v=>activeUserIds.has(v.user_id));
  ...
  return(
    <div>
        ...
        <div className="vf-search-box" style={{background:'var(--white)',borderRadius:20,padding:'28px 32px',width:'100%',maxWidth:780,boxShadow:'0 16px 60px rgba(0,0,0,0.28)',position:'relative',zIndex:10,marginBottom:16}}>
          <div className="vf-search-grid" style={{display:'grid',gridTemplateColumns:'3fr 2fr',gap:14,marginBottom:16,alignItems:'end'}}>
            <div>
              ...
              <VenueAutocomplete value={venue} onChange={setVenue} onPlaceSelected={(ll,name)=>{setVenueLatLng(ll);setVenue(name);}} placeholder="e.g. Babylonstoren (optional)" style={{border:'1.5px solid var(--parchment)',borderRadius:9,padding:'11px 14px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.9rem',color:'var(--charcoal)',background:'var(--cream)',outline:'none',width:'100%'}}/>
            </div>
      ...
      {/* Back to Plan banner */}
      {fromPlan&&onReturnToPlan&&(
        <div style={{background:'var(--forest)',padding:'10px 28px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:12}}>
          <span style={{fontSize:'0.84rem',color:'var(--gold-light)',display:'flex',alignItems:'center',gap:8}}>
            {IC.rings(14,'var(--gold-light)')} Browsing from your Wedding Plan
          </span>
          <button onClick={onReturnToPlan}
            style={{background:'var(--gold)',color:'var(--forest)',border:'none',borderRadius:8,padding:'6px 16px',fontSize:'0.8rem',fontWeight:600,cursor:'pointer',flexShrink:0}}>
            ← Back to Wedding Plan
          </button>
        </div>
      )}
      ...
      {/* Results */}
      {searched&&(
        <div ref={resultsRef} className="vf-results-section" style={{padding:'28px 0 20px',background:'#ffffff'}}>
          <div className="vf-results-header" style={{padding:'0 28px 20px',maxWidth:1200,margin:'0 auto',display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
            ...
            {vendorsWithLoc.length>0&&<button onClick={()=>setShowMap(s=>!s)} style={{background:'var(--parchment)',border:'1.5px solid var(--parchment)',borderRadius:8,padding:'7px 14px',fontFamily:"'DM Sans',sans-serif",fontSize:'0.78rem',color:'var(--forest)',cursor:'pointer',display:'flex',alignItems:'center',gap:5}}>{IC.map(14,'var(--forest)')}{showMap?'Hide map':'Show map'}</button>}
          </div>
          ...
          {!loading&&!loadError&&<VendorGrid vendorsByType={vendorsByType} activeTypes={activeTypes} dateFrom={dateFrom} dateTo={dateTo} onOpenDetail={openDetail} onRequestQuote={onRequestQuote} customerId={user?.customerId} planMaxPrices={planMaxPrices}/>}
